...
df.loc[2013:2014, [c for c in df.columns if c.startswith('gdp')]]
...
df['unemp_mean_na'] = df.filter(like='unemp').mean(axis=1, skipna=True).round(1)
df['gdp_sum_na']    = df.filter(like='gdp').sum(axis=1, skipna=True)
...
df_mexico = df.loc[2014:, [c for c in df.columns if c.endswith('mexico')]]
...
df_mexico['gdp_delta'] =  df_mexico.filter(like='gdp').diff(axis=0)
